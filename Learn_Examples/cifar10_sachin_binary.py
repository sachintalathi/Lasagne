...
import lasagne.binary_connect as bc ## Binary Connect Layer
from collections import OrderedDict


### Module to compute first order stats over entire data set #########
...
def batch_gen(X,y,N):
  #Will receive the training data batch tensor (Num images, Channels, Img_X, Img_Y) and
  #The training data batch labels (integers)
  #Will produce a generator for mini batches
  while True: #this statement allows for continuous looping over the input data stream
...
def simple_binary_cnn_network(input_var,batchnorm_bool=0, dropout_bool=0, node_type='ReLU'):
...
if __name__=="__main__":
  ...
  if opts.simple_cnn:
    if binary:
      net=simple_binary_cnn_network(X_sym,batchnorm_bool=opts.batch_norm)
    else:
      net=simple_cnn_network(X_sym,batchnorm_bool=opts.batch_norm)
  elif opts.cnn:
    net=cnn_network(X_sym,batchnorm_bool=opts.batch_norm)
  else:
    print ('No Network Defined')
    sys.exit(0)
          
      
  output=lasagne.layers.get_output(net['l_out'],X_sym,deterministic=False)
  ...
  post_train_values=lasagne.layers.get_all_param_values(net['l_out'])